...
of the following list of `r RFLOMICS:::omicsDic(dataset.SE)[["variableName"]]`  `r names(dataset.SE@metadata$CoExpAnal$gene.list.names)`. 
`r dataset.SE@metadata[["CoExpAnal"]]$method` was used to cluster the `r RFLOMICS:::omicsDic(dataset.SE)[["variableName"]]`. 
The gaussian mixture model was run on filtered (meanFilterCutoff was set to: `r dataset.SE@metadata[["CoExpAnal"]]$param$meanFilterCutoff`), 
normalized (normalization method was `r dataset.SE@metadata[["CoExpAnal"]]$param$normFactors`) 
and transformed (transformation method: `r dataset.SE@metadata[["CoExpAnal"]]$param$transformation`) count data.
...
tab.clusters <- as.data.frame(cosRes[[names(cosRes) == names(dataset.SE@metadata$CoExpAnal$cluster.nb)]])
