...
bioFact   <- RFLOMICS::getBioFactors(rflomics.MAE)
batchFact <- RFLOMICS::getBatchFactors(rflomics.MAE)
metaFact  <- RFLOMICS::getMetaFactors(rflomics.MAE)

DT::datatable(getDesignMat(rflomics.MAE)[c("samples", bioFact, batchFact, metaFact)], 
...
```
`r RFLOMICS::getModelFormula(rflomics.MAE)`
````
...
names(df) <- c("Comparison", "Contrast_names", "Type")
