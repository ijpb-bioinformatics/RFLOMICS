...
```{r MA-plot, out.width = "60%", fig.cap=paste0("<p style=\"color:grey\">","*fig: Expect a majority of ", RFLOMICS:::.omicsDic(dataset.SE)[["variableName"]]," around 0. The red points are the ", RFLOMICS:::.omicsDic(dataset.SE)[["variableName"]]," significantly over-expressed in the left factor's modality in the contrast expression whereas blue points are ",RFLOMICS:::.omicsDic(dataset.SE)[["variableName"]]," significantly under-expressed in the rigth factor's modality in the contrast expression. Only the top 20 ", RFLOMICS:::.omicsDic(dataset.SE)[["variableName"]]," DE are labeled.*", "</p>")}
diff.plots$MA.plot
```

#### VolcanoPlot

```{r Volcano-plot, out.width = "60%", fig.cap=paste0("<p style=\"color:grey\">","*fig: Red points are ", RFLOMICS:::.omicsDic(dataset.SE)[["variableName"]]," of interest displaying both large magnitude log-fold-changes (x axis) and high statistical significance (y axis). Only the top 20 ",RFLOMICS:::.omicsDic(dataset.SE)[["variableName"]]," DE are labeled.*", "</p>")}
diff.plots$Volcano.plot
```

#### Heatmap:

```{r heatmap-plot, out.width = "60%", fig.cap=paste0("<p style=\"color:grey\">","*fig: Heatmap was performed on DE ", RFLOMICS:::.omicsDic(dataset.SE)[["variableName"]]," expression data table which has been transformed by: ", getTransSettings(dataset.SE)$method," method and normalized by: ", getNormSettings(dataset.SE)$method," method. Clustering was independently performed on samples (row) and centered ", RFLOMICS:::.omicsDic(dataset.SE)[["variableName"]]," (column) using euclidian distance and complete aggregation method*", "</p>")}
...
DT::datatable(df, rownames = TRUE , class = 'cell-border stripe',
              options = list(pageLength = 6, scrollX = F, lengthChange = F)) |> 
  DT::formatStyle(
      'logFC',
      color = DT::styleInterval(c(0), c('red', 'green'))
    )
