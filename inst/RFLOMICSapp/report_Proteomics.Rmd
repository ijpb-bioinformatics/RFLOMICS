...
* The data are transformed using `r FlomicsMultiAssay@ExperimentList[[paste0(data,".filtred")]]@metadata$transform_method`.
...
* Distribution of protein counts before/after pre-processing : boxplot
...
* Distribution of protein counts before/after pre-processing : density
...
The following graph gives the number of common and specific differential expressed proteins per testing hypothesis.
...
`r if(is.null(dataset.SE@metadata[["DiffExpEnrichAnal"]]$results) && is.null(dataset.SE@metadata[["CoExpEnrichAnal"]]$results)) {"\\begin{comment}"}`

## Annotation and Enrichment

The Enrichment are performed using hypergeometric tests to find annotation terms that are specifically over or under-represented in a given list of proteins with respect to a reference specified by an annotation file.

`r if(is.null(dataset.SE@metadata[["DiffExpEnrichAnal"]]$results)) {"\\begin{comment}"}`

* On the lists of differential expressed proteins
...
```{r , message=FALSE, warning=FALSE, echo=FALSE}
  ...
  for (listname  in  names(dataset.SE@metadata[[annot]]$results)){
      out <- c(out, knitr::knit_child("report_06_annot_enrich_per_list.Rmd", quiet=TRUE))
  }
...
`r if(is.null(dataset.SE@metadata[["DiffExpEnrichAnal"]]$results)) {"\\end{comment}"}`

`r if(is.null(dataset.SE@metadata[["CoExpEnrichAnal"]]$results)) {"\\begin{comment}"}`

* On the co-expressed protein clusters
...
`r if(is.null(dataset.SE@metadata[["DiffExpEnrichAnal"]]$results) && is.null(dataset.SE@metadata[["CoExpEnrichAnal"]]$results))  {"\\end{comment}"}`
