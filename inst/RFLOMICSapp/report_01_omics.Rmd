...
for (analysisType in names(analyzedDatasetNames)){
  ...
  if (!analysisType %in% c("DiffExpEnrichAnal", "CoExpEnrichAnal")) {
    ...
    for(dataset in analyzedDatasetNames[[analysisType]]){
      
      if (analysisType == "DataProcessing"){
        explorDir[[dataset]] <- 
          paste0(outDir,"/", dataset, "/03_data_processing/")
        
        dir.create(path = explorDir[[dataset]],
                   recursive = TRUE, showWarnings = FALSE)
      }
      
      if (analysisType == "DiffExpAnal"){
        diffDir[[dataset]] <-
          paste0(outDir, "/", dataset,"/04_differential_analysis/")
        
        dir.create(path = diffDir[[dataset]],
                   recursive = TRUE, showWarnings = FALSE)
      }
      
      if (analysisType == "CoExpAnal"){
        coexpDir[[dataset]] <-
          paste0(outDir, "/", dataset, "/05_coExpression_analysis/")
        
        dir.create(path = coexpDir[[dataset]],
                   recursive = TRUE, showWarnings = FALSE)
      }
    }
  }else{
    ...
    for (database in names(analyzedDatasetNames[[analysisType]])) {
      ...
      for (dataset in analyzedDatasetNames[[analysisType]][[database]]) {
        
        annotDir[[dataset]][[analysisType]][[database]] <-
          paste0(outDir, "/", dataset, "/06_enrichment_analysis/",
                 analysisType, "/", database, "/")
        
        dir.create(path = annotDir[[dataset]][[analysisType]][[database]],
                   recursive = TRUE, showWarnings = FALSE)
      }
    }
  }
...
```{r diffAnalysesSummary, results='asis', eval=("DiffExpAnal" %in% analysisTypes), fig.cap=paste0("<p style=\"color:grey\">","", "</p>")}
...
```{r coExpAnalysesSummary, results='asis', eval=("CoExpAnal" %in% analysisTypes), fig.cap=paste0("<p style=\"color:grey\">","", "</p>")}
cat("## 5.1 Summary\n")
