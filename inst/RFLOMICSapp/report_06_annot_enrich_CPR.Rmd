...
annotDir <- paste0(dataDir,"/","04-Enrichment_analysis_CPR/", annot)
...
kableExtra::kable_styling(kableExtra::kable(RFLOMICS::sumORA(dataset.SE, 
                                                             from = annot,
                                                             database = ontology), 
                                            row.names=FALSE, caption=NULL, format="html"), 
                          font_size=12)
```

* **Diplay results by contrast(s)**

*Please click on the arrows to scroll down the results and click on the figure to zoom in*

```{r}
out <- NULL
for (listname  in names(RFLOMICS::getEnrichRes(dataset.SE,
                                               from = annot,
                                               database = ontology))){
      out <- c(out, knitr::knit_child(paste0(path.package("RFLOMICS"), 
                                             "/RFLOMICSapp/",
                                             "report_06_annot_enrich_per_list_per_Domain_CPR.Rmd"),
                                      quiet=TRUE))
}
