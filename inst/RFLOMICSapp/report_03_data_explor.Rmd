...
* Then, *genes* with a cpm lower to `r dataset.SE@metadata$FilteringOptions$CPM_Cutoff` in at least "`r dataset.SE@metadata$FilteringOptions$Filter_Strategy`" were filtered out ( `r length(dataset.SE@metadata$FilteredFeatures)` genes).
* The `r dataset.SE@metadata$Normalization$methode` normalization method was applied. 

`r if(dataset.raw.SE@metadata$omicType != "RNAseq") {"\\end{comment}"}`

`r if(dataset.SE@metadata$Normalization$methode == "none") {"\\begin{comment}"}`
...
`r if(dataset.SE@metadata$transform$transform_method == "none") {"\\begin{comment}"}`

* The data were transformed using `r dataset.raw.SE@metadata$transform_method`.

`r if(dataset.SE@metadata$transform$transform_method == "none") {"\\end{comment}"}`
