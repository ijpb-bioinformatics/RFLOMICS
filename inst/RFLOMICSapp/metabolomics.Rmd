...
Data_Distribution_Density.plot(FlomicsMultiAssay@ExperimentList[[paste0(dataset.SE)]])
Data_Distribution_Density.plot(FlomicsMultiAssay@ExperimentList[[paste0(dataset.SE,".filtred")]])
```
...
```{r PCAMetabo, echo=FALSE , fig.show="hold", message=FALSE, warning=FALSE,out.width = "80%", eval=FALSE}
...
used to cluster them with the following parameters. The gaussian mixture models (`r paste0("model=",dataset.SE@metadata[["CoExpAnal"]]$GaussianModel,"")` was run on scaled by metabolits data to cluster them by expression profile rather than abundance.
