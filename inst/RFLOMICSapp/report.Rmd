...
author: "Analyzis were performed by the RFLOMICS package (cite)"
...
```{r analysis, eval=(length(rflomics.MAE@metadata$omicList) != 0 )}
...
`r if(is.null(rflomics.MAE@metadata$MOFA)) {"\\begin{comment}"}`

# MOFA2 Integration

`r if(is.null(rflomics.MAE@metadata$MOFA)) {"\\end{comment}"}`

```{r, eval = !is.null(rflomics.MAE@metadata$MOFA), hide = is.null(rflomics.MAE@metadata$MOFA) }
...
`r if(is.null(rflomics.MAE@metadata$mixOmics)) {"\\begin{comment}"}`

# MixOmics Integration

`r if(is.null(rflomics.MAE@metadata$mixOmics)) {"\\end{comment}"}`

```{r, eval = !is.null(rflomics.MAE@metadata$mixOmics), hide = is.null(rflomics.MAE@metadata$mixOmics) }

out <- NULL
for (selectedResponse in names(rflomics.MAE@metadata$mixOmics)) { 
