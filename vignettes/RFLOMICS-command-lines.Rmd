...
Design.Factors.Ref <- c("rep1","Medium","DS")
...
```{r creat MAE}
 ...
 FlomicsMultiAssay <- FlomicsMultiAssay.constructor(inputs      = ListofData, 
                                                    refList = Design.Factors.Ref, typeList = Design.Factors.Type,ExpDesign = ExpDesign,
...
```{r CheckDesign, fig.cap="Design checking's result",fig.small=TRUE}
  ...
  # check completeness
  completeCheckRes <- CheckExpDesignCompleteness(FlomicsMultiAssay)
...
```{r viewContrast  }
 ...
 Design <- getExpressionContrast(object = FlomicsMultiAssay, 
...
```{r SetContrast}
 ...
 # Select The 2 firsts
 Design.contrastList <- FlomicsMultiAssay@metadata$design@Contrasts.List
 ...
 Design <- getContrastMatrix(object = FlomicsMultiAssay, contrastList = unlist(Design.contrastList))
...
<!-- * dans le cas ou le nombre de samples dans la matrice ne correspond tous à fait aux samples decrits dans le design -->
<!-- il faut faire une fonction pour aider l'utilisateur de retirer des samples facilement -->
```{r CheckExp }
CheckCompleteness.dataset <- CheckExpDesignCompleteness(FlomicsMultiAssay, colnames(RNAseq.raw.SE))
...
To perform annotation and enrichment analysis, we need Reference file (see [rflomics input data](`r file.path(path = path.package("RFLOMICS"), file ="/docs/articles/RFLOMICS-input-data.html")`). 
...
ListNames.coseq <- names(RNAseq.SE@metadata$CoExpAnal[["clusters"]])

```

Run gene enrichment with hypergenometric method.
```{r, runAnnotationEnrichment}
...
```{r, pvalue.enrichment.plot overrepresented}
...
Plot the over-represented GO terms for each list.
```{r, pvalue.enrichment.plot underrepresented}
