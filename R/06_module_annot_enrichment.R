...
module_runEnrichment <- function(input, output, session, dataset, dom.select, list.source, rea.values, local.rea.values){
  ...
  output$summary <- renderUI({
    ...
    if (rea.values[[dataset]][[diffAnnot_coExpAnal]] == FALSE || is.null(session$userData$FlomicsMultiAssay[[dataset]]@metadata[[list.source]][[dom.select]])) return()
  ...
  output$AnnotResults<- renderUI({
    ...
    if (rea.values[[dataset]][[diffAnnot_coExpAnal]] == FALSE || is.null(local.rea.values$dataset.SE@metadata[[list.source]][[dom.select]][["summary"]])) return()
...
AnnotationEnrichmentClusterProf <- function(input, output, session, dataset, rea.values){
  ...
  observeEvent(input$runEnrich_CPR_GO, {
    ...
    if (length(input$GeneList.diff_GO) != 0) {
      ...
      # run annotation
      local.rea.values$dataset.SE <-  runAnnotationEnrichment(object = local.rea.values$dataset.SE, 
    ...
    if (length(input$GeneList.coseq_GO) != 0) {
      ...
      local.rea.values$dataset.SE <-  runAnnotationEnrichment(local.rea.values$dataset.SE, 
    ...
    local.rea.values[["GO"]] <- TRUE
    
    session$userData$FlomicsMultiAssay[[dataset]] <- local.rea.values$dataset.SE
    
    shiny::callModule(module  = module_runEnrichment, id = "GO_DiffExpEnrichAnal", dataset = dataset, dom.select = "GO", list.source = "DiffExpEnrichAnal",
                      rea.values = rea.values, local.rea.values = local.rea.values)
    
    shiny::callModule(module  = module_runEnrichment, id = "GO_CoExpEnrichAnal", dataset = dataset, dom.select = "GO", list.source = "CoExpEnrichAnal",
                      rea.values = rea.values, local.rea.values = local.rea.values)
  ...
  observeEvent(input$runEnrich_CPR_KEGG, {
    ...
    toto <<- local.rea.values$dataset.SE
...
#'           # select DE list
#'           pickerInput(
#'             inputId = ns("GeneList.diff"), label = "Select DE lists:", 
#'             choices = ListNames.diff, multiple = TRUE, selected = ListNames.diff,
#'             options = list(`actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3")
#'           ),
#'           
...
#'     output$selectColumnsFromCustomAnnot <- renderUI({
#'       
#'       annotation <- data.table::fread(file = input$annotationFileCPR$datapath, 
#'                                       sep = "\t", header = TRUE)
#'       
#'       column(width = 12,
#'              
