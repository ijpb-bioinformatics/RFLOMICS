    ...
    function(object.SE,
        ...
        if (!is.null(paramList$diffList)) {
            ...
            if (!(setequal(paramList$domain, list_args$domain)))
                return(TRUE)
            if (paramList$pvalueCutoff != list_args$pvalueCutoff)
                return(TRUE)
            
            switch(dbType,
                   "GO" = {
                       if (paramList$OrgDb    != list_args$OrgDb)
                           return(TRUE)
                       if (paramList$keyType  != list_args$keyType)
                           return(TRUE)
                   },
                   "KEGG" = {
                       if (paramList$keyType  != list_args$keyType)
                           return(TRUE)
                       if (paramList$organism != list_args$organism)
                           return(TRUE)
                   })
        }
