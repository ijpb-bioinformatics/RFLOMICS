...
#' @noRd
methods::setMethod(f          = "plotLibrarySize",
                   ...
                   definition <- function(object, raw = FALSE){
                     ...
                     if (raw) {
                       ...
                       if (!.isNorm(object)) pseudo <- SummarizedExperiment::assay(.applyNorm(object)) 
                       
                       pseudo <- pseudo %>% colSums(., na.rm = TRUE)
                       title <- paste0("Filtered and normalized (", getNormSettings(object)$method, ") data")
                     }
...
methods::setMethod(f          = "plotLibrarySize",
                   ...
                   definition = function(object, SE.name, raw = FALSE){
                     ...
                     if (getOmicsTypes(object[[SE.name]]) == "RNAseq") {
                       return( plotLibrarySize(object[[SE.name]], raw = raw))
                     }else{
  ...
  definition = function(object, plot = "boxplot", raw = FALSE) {
    ...
    if (raw) {
      ...
      if (.isTransformed(object2)) {
        title <- paste0("Transformed (", getTransSettings(object2)$method, ") ", title)
      }
      
      if (.isNorm(object2)) {
        title <- paste0(title, " - normalization: ", getNormSettings(object2)$method)
      }
...
#'
#' @param object An object of class \link{RflomicsSE}
#' @param transformMethod The transformation to store in the metadata or to store and apply if modifyAssay is TRUE.
#' @param modifyAssay Boolean. Do the transformation need to be applied on the data? The raw data will be replaced by the transformed ones.
#'
...
#' 
methods::setMethod(f          = "runTransformData",
                   signature  = "RflomicsSE",
                   definition = function(object, transformMethod = NULL, modifyAssay = FALSE){
...
methods::setMethod(f          = "runTransformData",
                   signature  = "RflomicsMAE",
                   definition = function(object, SE.name, transformMethod = NULL, modifyAssay = FALSE){
...
methods::setMethod(f          = "getTransSettings",
                   signature  = "RflomicsSE",
...
methods::setMethod(f          = "filterLowAbundance",
                   signature  = "RflomicsMAE",
                   definition = function(object, SE.name, filterMethod= "CPM", filterStrategy = "NbConditions", cpmCutoff = 5){
...
methods::setMethod(f          = "getFilterSettings",
                   signature  = "RflomicsSE",
...
methods::setMethod(f          = "getFilteredFeatures",
                   signature  = "RflomicsSE",
...
methods::setMethod(f          = "getCoeffNorm",
                   signature  = "RflomicsSE",
...
methods::setMethod(f          = "getNormSettings",
                   signature  = "RflomicsSE",
...
#' @param normMethod of normalisation. Mandatory for RNAseq data. Default value : RNAseq = TMM.
