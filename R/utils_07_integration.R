...
MOFACorNetwork <- function(resMOFA,
    ...
    if (nrow(feature_filtered) > 0) {
        if (is.null(omics_colors)) {
            omics_colors <-
                as.list(rownames(brewer.pal.info)[seq_len(length(unique(feature_filtered$Table)))])
            names(omics_colors) <- unique(feature_filtered$Table)
        }
        
        omics_colors <- lapply(
            unique(feature_filtered$Table),
            FUN = function(omicTable) {
                brewer.pal(name = omics_colors[[omicTable]],
                           n = 5)
            }
        )
        names(omics_colors) <- unique(feature_filtered$Table)
        ...
        feature_filtered$Color2 <- unlist(lapply(
            ...
            FUN = function(i) {
                index <- as.numeric(feature_filtered$Color[i])
                omics_colors[[feature_filtered$Table[i]]][index]
            }
        ))
        ...
        if (any(abs(cor_display[upper.tri(cor_display)]) >= abs_min_cor_network)) {
            ...
            gg.legend <-
                ggplot(legend.reshape, aes(x = Var2, y = Var1)) +
            ...
            # Actual plotting
            ggarrange(
                plotlist = list(qgraph_plot, gg.legend),
                nrow = 1,
                ncol = 2,
                widths = c(3, 1)
            )
