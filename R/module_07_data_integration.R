...
#' @keywords internal
#' @noRd
.modIntegrationAnalysis <- function(input, output, session, rea.values, method){
    ...
    switch(
      ...
      "mixOmics" = {
        box(title = span(tagList(icon('chart-line'), "   ", 
            ...
            p("RNASeq data, given in the form of counts, will be processed using limma::voom transformation. 
              If you have indicated a batch effect when loading your data,
              it will be corrected in all datatables using limma::removebatcheffect before running mixOmics, f
              or each table."),
            ...
            p("- Sparse Analysis: if checked, function block.splsda is run, a variable selection 
              is performed for each component and each response variable"),
            p("- Tuning cases: if \"sparse analysis\" is checked, to select the relevant features for each component, 
              tuning has to be performed. Tuning cases determines the number of feature selection
              to try and decide on. A little warning here: tuning cases are applied on each table and component, 
              and all combinaisions are tested (for example: two datatables and five tuning cases will 
              make 5*5 cases for each component to test), it can be quite long."),
            
            h4(tags$span("Response variables:", style = "color:orange")),
            p("You can select as many response variables as you want, 
            ...
            p("- Data Overview: shows how many samples and omic features are 
            ...
            p("- Explained variance: two graphs are displayed in this section. 
              ...
              The second one is a detailed version, showing the percentage of 
              explained variance per feature per omic data."), 
            p("- Individuals plot: coordinates of individuals on each component for each table. 
              When toggled, ellipses force all windows to be on the same scale."),
            p("- Features plot: similar to a pca correlation plot, coordinates of the (selected) 
              features on the correlation circles for all datasets. "),
            p("- Loadings: coefficients for the (selected) features for each dataset,
              by ascending order. Default is showing the first 25 entities for each block, on the first component."),
            p("- Networks: similarity networks computed on selected components factors. 
              It only shows the between table correlation, not the intra-table correlaction!"),
            p("- CircosPlot: only available for sparse analyses. 
                Similarity networks computed on selected components factors. 
              It only shows the between table correlation, not the intra-table correlaction!"),
      ...
      "MOFA" = {
        box(title = span(tagList(icon('chart-line'), "   ", a("MOFA+", href = "https://biofam.github.io/MOFA2/"),
            ...
            p("RNASeq data, given in the form of counts, will be processed using limma::voom transformation. 
              If you have indicated a batch effect when loading your data,
              it will be corrected in all datatables using limma::removebatcheffect before running MOFA."),
            
            h4(tags$span("Outputs:", style = "color:orange")),
            p("- Data Overview: shows how many samples and omic features are left per table after applying all filters. 
              These are the data on which MOFA is performed."),
            p("- Factors correlation: shows the correlation between factors. 
              It is essential that the factors are decorrelated between each other. 
              If there is a correlation >0.5 in absolute value, 
            ...
            p("- Heatmap: shows the data for the selected number of features, per factor and per omic. 
  ...
  observeEvent(input$run_prep, {
    ...
    if (length(lowNbVarTab) != 0) {
      showModal(modalDialog(title = "ERROR: ", 
                            paste0("number of variables is lower than 5 in this(these) table(s): ",
                                   lowNbVarTab)))
    }
    validate({ 
  ...
  renderUI({
    ...
    box(title = span(tagList(icon("sliders"), "  ", "Settings")), 
        ...
        switch(
          ...
          "mixOmics" = {
            list(
              column(12, checkboxInput(inputId = ns("scale_views"), 
                                       label = "Scale Datasets",
                                       value = TRUE, width = NULL)),
              column(12, checkboxInput(inputId = ns("MO_sparsity"), 
                                       label = "Sparse analysis", 
                                       value = FALSE, width = NULL)),
              column(12, numericInput(inputId = ns("MO_ncomp"), 
                                      label = "Components", 
                                      value = 5, min = 1, max = 20)),
              column(12, numericInput(inputId = ns("MO_cases_to_try"), 
                                      label = "Tuning cases",
          ...
          "MOFA" = {
            list(
              column(12,
                     selectInput(ns("scale_views"),
                                 label    = "Scale views",
                                 choices  = c(FALSE, TRUE),
                                 selected = TRUE)),
              column(12,
                     numericInput(inputId = ns("MOFA_numfactor"), 
                                  label = "Num factors:", 
                                  value = 10, min = 5, max = 15)),
              column(12,
                     numericInput(inputId = ns("MOFA_maxiter"), 
                                  label = "Max iteration:", 
                                  value = 1000, min = 1000, max = 1000))
            )
          }
        ),
        column(12, actionButton(ns("run_integration"), "Run Analysis"))
    )
  })
}
...
.integrationPrepareParamUI <- function(session, input){
  ...
  renderUI({
    ...
    lapply(input$selectData, function(set){
      ...
      ValidContrasts <- getValidContrasts(
      ...
      box(title = set, width = 4, background = "green",
          ...
          radioButtons(inputId = ns(paste0("selectmethode", set)), 
          ...
          conditionalPanel(
            ...
            radioButtons(inputId = ns(paste0("unionORintersect", set)), 
                         label = NULL, 
                         choiceNames = c("union", "intersection"), 
                         choiceValues = c(TRUE, FALSE),
                         selected = TRUE, inline = TRUE)
          ),
          if (getOmicsTypes(userData$FlomicsMultiAssay[[set]]) == "RNAseq") {
            selectInput(inputId  = ns("RNAseqTransfo"),
...
.outMONetwork <- function(session, input, settings, Response, Data_res){
  ...
  fluidRow(
    column(1, numericInput(inputId = ns(paste0(Response, "Network_cutoff")),
                           label = "Cutoff:",
                           min = 0,
                           max =  1,
                           value = 0.9, step = 0.05)),
...
.outMOCimPlot <- function(session, input, settings, 
  ...
  if (is(Data_res, "block.splsda")) {
    fluidRow(
      column(1,
             numericInput(inputId = ns(paste0(Response, "cimComp")),
                          label = "Comp",
                          min = 1,
                          max = settings$ncomp,
                          value = 1, step = 1)),
...
.outMOFAWeightPlot <- function(session, resMOFA, input){
  ...
  renderUI({
    verticalLayout(
      ...
      fluidRow(
        column(3, 
               sliderInput(inputId = ns("WeightsPlot_Factors_select"),
                           label = 'Factors:',
                           min = 1, 
        ...
        column(2, 
               numericInput(inputId = ns("nfeat_choice_WeightsPlot"),
                            label = "Features:",
                            min = 1,
                            max = 500,
        ...
        column(1,
               checkboxInput(inputId = ns("scale_choice_WeightsPlot"), 
                             label = "Scale Weights", 
                             value = FALSE, 
                             width = NULL)
        ),
...
.outMOFAWeightTable <- function(session, resMOFA, input){
  ...
  verticalLayout(
    fluidRow(
      column(11, sliderInput(inputId = ns("Factors_select_MOFA"),
                             label = 'Factors:',
                             min = 1, 
                             max = resMOFA@dimensions$K,  
                             value = c(1,2), step = 1)) 
...
.outMOFAFactorsPlot <- function(session, resMOFA, input){
  ...
  renderUI({
    verticalLayout(
      fluidRow(
        column(2, sliderInput(inputId = ns("factors_choices_MOFA"),
                              label = 'Factors:',
                              min = 1, 
                              max = resMOFA@dimensions$K, 
                              value = c(1,2), step = 1)),
        column(2, radioButtons(inputId = ns("color_by_MOFA"),
                               label = "Color:",
                               choices = c('none', moreChoices),
                               selected = "none")
        ),                           
        column(2, radioButtons(inputId = ns("shape_by_MOFA"),
                               label = "Shape:",
                               choices = c('none', moreChoices),
                               selected = "none")
        ),                                  
        column(2, radioButtons(inputId = ns("group_by_MOFA"),
                               label = "Group by:",
                               choices = c('none', moreChoices),
                               selected = "none")
        ),                           
        column(1,
               fluidRow(checkboxInput(inputId = ns("add_violin_MOFA"), 
                                      label = "Violin", 
                                      value = FALSE, width = NULL)),
               fluidRow(checkboxInput(inputId = ns("add_boxplot_MOFA"), 
                                      label = "Boxplot", 
                                      value = FALSE, width = NULL)),
               fluidRow(checkboxInput(inputId = ns("scale_scatter_MOFA"), 
                                      label = "Scale factors", 
                                      value = FALSE, width = NULL))
        )),            
      fluidRow(
  ...
  renderUI({
    verticalLayout(
      ...
      fluidRow(# buttons - choices for heatmap
        ...
        column(1, 
               numericInput(inputId = ns("factor_choice_heatmap_MOFA"),
                            label = "Factor:",
                            min = 1,
                            max =  resMOFA@dimensions$K,
                            value = 1, step = 1)),
        column(1,),
        column(2, 
               radioButtons(inputId = ns("view_heatmap_MOFA"),
                            label = "Data:",
        ...
        column(1,),
        column(2, 
               numericInput(inputId = ns("nfeat_choice_heatmap_MOFA"),
                            label = "Features:",
                            min = 1,
                            max = 500,
        ...
        column(1,
               checkboxInput(inputId = ns("denoise_heatmap_MOFA"), 
                             label = "Denoise", value = FALSE, width = NULL)
        ),
        column(1,),
        column(2, 
               radioButtons(inputId = ns("annot_samples_MOFA"),
                            label = "Annotation:",
                            choices = c('none', 
      ...
      fluidRow(
        column(12, # heatmap is too large with just renderPlot. 
               renderPlot({
                 ...
                 maxSlider <- get_dimensions(resMOFA)$D[input$view_heatmap_MOFA][[1]]
  ...
  renderUI({
    verticalLayout(
      fluidRow(# buttons - choices for network
        column(1, 
               numericInput(inputId = ns("factor_choice_network"),
                            label = "Factor:",
                            min = 1,
                            max =  resMOFA@dimensions$K,
                            value = 1, step = 1)),
        column(2, 
               numericInput(inputId = ns("abs_weight_network"),
        ...
        column(2, 
               numericInput(inputId = ns("abs_min_cor_network"),
        ...
        column(2, 
               radioButtons(inputId = ns("network_layout"),
                            label = "Layout:",
        ...
        column(1,  
               fluidRow(colourInput(inputId = ns("posCol"),
                                    ...
                                    returnName = FALSE,
                                    closeOnClick = FALSE)),
               fluidRow(colourInput(inputId = ns("negCol"),
        ...
        column(2,  lapply(seq_len(length(views_names(resMOFA))), function(i) {
          selectInput(inputId = ns(paste0("colors_", views_names(resMOFA)[i])),
      ...
      fluidRow(
        column(12,
               renderPlot({
                 ...
                 MOFACorNetwork(resMOFA = resMOFA, 
...
.outMOFARelations <- function(resMOFA){
  renderTable({
    ...
    res_aov <- lapply(seq_len(ncol(ExpDesign)), FUN = function(i){
      ...
      unlist(lapply(summary(aov1), FUN = function(list_res){
        list_res[["Pr(>F)"]][[1]]
      }))
    })
